...
const DEFAULT_CATEGORIES = ['Labs', 'Quizzes', 'Assignments']
...
export default function AddTask({ onTaskAdded }) {
  ...
  const selectedPriority = PRIORITY_OPTIONS.find((p) => p.label === priority)

  function validate() {
    const e = {}
    if (!task.trim()) e.task = 'Task name is required.'
    if (!priority) e.priority = 'Please select a priority.'
    if (!deadline) e.deadline = 'Please pick a deadline.'
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (onTaskAdded) onTaskAdded(saved)
      setTask('')
      setPriority('')
      setDeadline('')
      setReminder('')
      ...
      setErrors({ submit: 'Could not save — is json-server running on port 3001?' })
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50 px-6 py-8">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-1">
          <CirclePlus size={18} className="text-amber-500" />
          <h1 className="text-sm font-semibold text-gray-800">Add Task</h1>
        </div>
          ...
          <div className="relative">
            ...
            >
              <span className="flex items-center gap-2.5">
                {selectedPriority ? (
                  <>
                    <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${selectedPriority.dot}`} />
                    <span className={`font-semibold ${selectedPriority.text}`}>
              ...
              <div className="absolute top-[calc(100%+6px)] left-0 right-0 z-50 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                {PRIORITY_OPTIONS.map((opt) => (
                  <button
                    key={opt.label}
                    type="button"
                    onClick={() => { setPriority(opt.label); setShowDropdown(false) }}
                    className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm transition duration-100 ${opt.hoverBg} ${priority === opt.label ? opt.activeBg : ''}`}
                  >
                    <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${opt.dot}`} />
                    <span className={`font-semibold ${opt.text}`}>{opt.label}</span>
                    <span className="text-gray-400 text-xs ml-auto">{opt.hint}</span>
                  </button>
                ))}
              </div>
            )}
